...
class CodexAuthState:
    ...
    def public(self, *, include_device_code: bool = False) -> dict[str, Any]:
        out = {
            "status": self.status,
        ...
        if include_device_code:
            out["verification_url"] = self.verification_url
...
class CodexAuthService:
    ...
    def __init__(
        ...
        *,
        ...
        log.debug(
            "Codex auth service configured: enabled=%s codex_bin_from_env=%s",
            self.enabled,
    ...
    def _bin_status(self) -> dict[str, Any]:
    ...
    async def _run_command(self, args: list[str], timeout: float = 15.0) -> tuple[int, str]:
        ...
        if not bin_path:
            log.warning("Codex command skipped because CLI is unavailable")
    ...
    async def status(self) -> dict[str, Any]:
        ...
        if not caps["codex_cli_available"]:
            log.warning("Codex auth status reports missing CLI")
    ...
    async def start(self) -> dict[str, Any]:
        ...
        bin_path = self._bin_path()
        if not bin_path:
            log.warning("Codex auth start reports missing CLI")
            return {**caps, **self._cli_unavailable_state(caps).public()}
        ...
        async with self._lock:
            if self._process is not None and self._process.returncode is None:
                return {**caps, **self._state.public(include_device_code=True)}
            self._state = CodexAuthState(
                status="starting",
        ...
        return {**caps, **(await self._wait_for_start_result()).public(include_device_code=True)}

    async def _wait_for_start_result(self) -> CodexAuthState:
        deadline = time.time() + self.code_timeout_seconds
    ...
    async def logout(self) -> dict[str, Any]:
        ...
        caps = self._base_capabilities()
        if not caps["codex_cli_available"]:
            log.warning("Codex logout reports missing CLI")
            return {**caps, **self._cli_unavailable_state(caps).public()}
